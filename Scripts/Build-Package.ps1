...
$Bridge = Join-Path $RepoRoot 'Skin\SonyXM5\@Resources\Bridge\SonyXM5Bridge-0.3.34.bin'
$BundledBuilder = Join-Path $RepoRoot '.build\rmskin-tools\bin\rmskin-builder.exe'

if (-not (Test-Path $Bridge)) {
    throw 'SonyXM5Bridge-0.3.34.bin is missing. Run Scripts\Build-Bridge.ps1 first.'
}
...
Get-ChildItem (Join-Path $StageSkin '@Resources\Bridge') -File -Filter 'SonyXM5Bridge*' -ErrorAction SilentlyContinue |
    Where-Object Name -ne 'SonyXM5Bridge-0.3.34.bin' |
    Remove-Item -Force
